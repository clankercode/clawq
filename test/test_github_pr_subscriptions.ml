...
let test_add_subscription () =
  with_db (fun db ->
      let sub =
        Github_pr_subscriptions.add ~db ~room_id:"room-1"
          ~repo:"owner/repo" ~pr_number:42 ~profile_id:1 ()
      in
      ...
      Alcotest.(check bool) "on_open default" true sub.notification_preferences.on_open;
      Alcotest.(check bool) "on_close default" true sub.notification_preferences.on_close;
      Alcotest.(check bool) "on_comment default" true sub.notification_preferences.on_comment;
      Alcotest.(check bool) "on_review default" true sub.notification_preferences.on_review;
      Alcotest.(check bool) "on_status default" true sub.notification_preferences.on_status;
      Alcotest.(check bool) "on_merge default" true sub.notification_preferences.on_merge)
...
let test_add_with_custom_preferences () =
  with_db (fun db ->
      ...
      Alcotest.(check bool) "on_close" false sub.notification_preferences.on_close;
      Alcotest.(check bool) "on_comment" true sub.notification_preferences.on_comment;
      Alcotest.(check bool) "on_review" false sub.notification_preferences.on_review;
      Alcotest.(check bool) "on_status" true sub.notification_preferences.on_status;
      Alcotest.(check bool) "on_merge" false sub.notification_preferences.on_merge)
...
let test_add_upserts_existing () =
  with_db (fun db ->
      ...
      let sub2 =
        Github_pr_subscriptions.add ~db ~room_id:"room-1"
          ~repo:"owner/repo" ~pr_number:42 ~profile_id:2 ()
      in
...
let test_find () =
  with_db (fun db ->
      ...
      in
      let found =
        Github_pr_subscriptions.find ~db ~room_id:"room-1"
...
let test_remove () =
  with_db (fun db ->
      ...
      in
      let removed =
        Github_pr_subscriptions.remove ~db ~room_id:"room-1"
          ~repo:"owner/repo" ~pr_number:42
      in
...
let test_update_preferences () =
  with_db (fun db ->
      ...
      in
      Alcotest.(check bool) "on_open" false updated.notification_preferences.on_open;
      Alcotest.(check bool) "on_close" false updated.notification_preferences.on_close;
      Alcotest.(check bool) "on_comment" false updated.notification_preferences.on_comment;
      Alcotest.(check bool) "on_review" false updated.notification_preferences.on_review;
      Alcotest.(check bool) "on_status" false updated.notification_preferences.on_status;
      Alcotest.(check bool) "on_merge" true updated.notification_preferences.on_merge)

let test_update_preferences_not_found () =
  with_db (fun db ->
      let new_prefs = Github_pr_subscriptions.default_notification_preferences in
      match
        Github_pr_subscriptions.update_preferences ~db ~room_id:"room-1"
          ~repo:"owner/repo" ~pr_number:99 ~preferences:new_prefs
      with
      | _ -> Alcotest.fail "expected Not_found"
      | exception Not_found -> Alcotest.(check bool) "raised Not_found" true true)
...
let test_find_by_room () =
  with_db (fun db ->
      ...
      let _sub2 =
        Github_pr_subscriptions.add ~db ~room_id:"room-1"
          ~repo:"owner/repo" ~pr_number:2 ~profile_id:1 ()
      in
      let _sub3 =
        Github_pr_subscriptions.add ~db ~room_id:"room-2"
          ~repo:"owner/repo" ~pr_number:1 ~profile_id:2 ()
      in
      let room1_subs =
...
let test_find_by_repo () =
  with_db (fun db ->
      let _sub1 =
        Github_pr_subscriptions.add ~db ~room_id:"room-1"
          ~repo:"owner/repo1" ~pr_number:1 ~profile_id:1 ()
      in
      let _sub2 =
        Github_pr_subscriptions.add ~db ~room_id:"room-2"
          ~repo:"owner/repo1" ~pr_number:2 ~profile_id:2 ()
      in
      let _sub3 =
        Github_pr_subscriptions.add ~db ~room_id:"room-1"
          ~repo:"owner/repo2" ~pr_number:1 ~profile_id:1 ()
      in
      let repo1_subs =
...
let test_find_by_repo_pr () =
  with_db (fun db ->
      ...
      let _sub3 =
        Github_pr_subscriptions.add ~db ~room_id:"room-1"
          ~repo:"owner/repo" ~pr_number:99 ~profile_id:1 ()
      in
      let pr42_subs =
...
let test_count () =
  with_db (fun db ->
      ...
      let _sub2 =
        Github_pr_subscriptions.add ~db ~room_id:"room-2"
          ~repo:"owner/repo" ~pr_number:2 ~profile_id:2 ()
      in
...
let test_should_notify () =
  ...
  Alcotest.(check bool) "opened" true (should_notify ~subscription:sub ~event_type:"opened");
  Alcotest.(check bool) "reopened" true (should_notify ~subscription:sub ~event_type:"reopened");
  Alcotest.(check bool) "closed" false (should_notify ~subscription:sub ~event_type:"closed");
  Alcotest.(check bool) "comment" true (should_notify ~subscription:sub ~event_type:"comment");
  Alcotest.(check bool) "review" false (should_notify ~subscription:sub ~event_type:"review");
  Alcotest.(check bool) "status" true (should_notify ~subscription:sub ~event_type:"status");
  Alcotest.(check bool) "merged" false (should_notify ~subscription:sub ~event_type:"merged");
  Alcotest.(check bool) "unknown event" true (should_notify ~subscription:sub ~event_type:"labeled")
