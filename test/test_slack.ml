...
let test_is_allowed_wildcard () =
  ...
  Alcotest.(check bool) "wildcard allows any" true
    (Slack.is_allowed ~config ~channel_id:"C123" ~user_id:"U456");
  Alcotest.(check bool) "wildcard allows other" true
    (Slack.is_allowed ~config ~channel_id:"CXXX" ~user_id:"UYYY")

let test_is_allowed_specific () =
  let config = make_config ~allow_channels:[ "C123" ] ~allow_users:[ "U456" ] () in
  Alcotest.(check bool) "matching ids" true
    (Slack.is_allowed ~config ~channel_id:"C123" ~user_id:"U456");
  Alcotest.(check bool) "wrong channel" false
...
let test_verify_signature_invalid () =
  ...
  let timestamp = Printf.sprintf "%.0f" (Unix.gettimeofday ()) in
  let body = {|{"type":"event_callback"}|} in
  Alcotest.(check bool) "invalid signature" false
    (Slack.verify_signature ~signing_secret ~timestamp ~body ~signature:"v0=deadbeef")
...
let test_verify_signature_expired () =
  ...
  let signature =
    "v0=" ^ Digestif.SHA256.(hmac_string ~key:signing_secret basestring |> to_hex)
  in
  Alcotest.(check bool) "expired timestamp" false
    (Slack.verify_signature ~signing_secret ~timestamp ~body ~signature)

let test_parse_url_verification () =
  let body = {|{"type":"url_verification","token":"tok","challenge":"abc123"}|} in
...
let suite =
  [
    ...
    Alcotest.test_case "verify_signature valid" `Quick test_verify_signature_valid;
    Alcotest.test_case "verify_signature invalid" `Quick test_verify_signature_invalid;
    Alcotest.test_case "verify_signature expired" `Quick test_verify_signature_expired;
