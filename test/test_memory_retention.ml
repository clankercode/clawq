...
let test_ensure_integrity_drops_anthropic_raw_only_orphan () =
  let tc = make_tool_call "tc-anth" "file_read" in
  let assistant_with_anthropic_raw =
    {
      (make_assistant_with_tool_calls [ tc ]) with
      provider_response_items_json =
        Some
          {|{"content":[{"type":"tool_use","id":"tc-anth","name":"file_read","input":{"path":"secret.ml"}}],"model":"claude-3","stop_reason":"tool_use"}|};
    }
  in
  let user_msg = Provider.make_message ~role:"user" ~content:"hi" in
