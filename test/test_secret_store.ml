...
let test_derive_key_different () =
  ...
  Alcotest.(check bool) "different passphrases different keys" true (key1 <> key2)
...
let test_encrypt_decrypt_roundtrip () =
  ...
  | Error msg ->
    Alcotest.fail (Printf.sprintf "decrypt failed: %s" msg)
...
let test_is_encrypted () =
  Alcotest.(check bool) "enc prefix" true
    (Secret_store.is_encrypted "$ENC:abc123");
  Alcotest.(check bool) "env var" false
    (Secret_store.is_encrypted "$MY_KEY");
  Alcotest.(check bool) "plain" false
    (Secret_store.is_encrypted "plain-key");
  Alcotest.(check bool) "empty" false
    (Secret_store.is_encrypted "")

let test_encrypt_secret_prefix () =
  let key = Secret_store.derive_key ~passphrase:"test" in
  let result = Secret_store.encrypt_secret ~key "my-api-key" in
  Alcotest.(check bool) "has ENC prefix" true
    (String.length result > 5 && String.sub result 0 5 = "$ENC:");
  Alcotest.(check bool) "is_encrypted" true
    (Secret_store.is_encrypted result)
...
let test_decrypt_secret_prefix () =
  ...
  | Error msg ->
    Alcotest.fail (Printf.sprintf "decrypt_secret failed: %s" msg)

let test_decrypt_secret_passthrough () =
  let key = Secret_store.derive_key ~passphrase:"test" in
  match Secret_store.decrypt_secret ~key "not-encrypted" with
  | Ok result ->
    Alcotest.(check string) "passthrough" "not-encrypted" result
  | Error msg ->
    Alcotest.fail (Printf.sprintf "passthrough failed: %s" msg)
...
let test_encrypt_config_secrets () =
  ...
  | Ok new_json ->
    ...
    let openai_key =
      new_json |> member "providers" |> member "openai" |> member "api_key" |> to_string
    in
    let groq_key =
      new_json |> member "providers" |> member "groq" |> member "api_key" |> to_string
    in
    (* openai key should be encrypted *)
    Alcotest.(check bool) "openai encrypted" true (Secret_store.is_encrypted openai_key);
    ...
    (match Secret_store.decrypt_secret ~key openai_key with
     | Ok decrypted -> Alcotest.(check string) "round-trip" "sk-real-key" decrypted
     | Error msg -> Alcotest.fail msg)
...
let test_empty_plaintext () =
  ...
  match Secret_store.decrypt ~key encrypted with
  | Ok decrypted ->
    Alcotest.(check string) "empty roundtrip" "" decrypted
  | Error msg ->
    Alcotest.fail (Printf.sprintf "empty decrypt failed: %s" msg)

let suite =
  [
    Alcotest.test_case "derive key length" `Quick test_derive_key;
    Alcotest.test_case "derive key deterministic" `Quick test_derive_key_deterministic;
    ...
    Alcotest.test_case "decrypt_secret passthrough" `Quick test_decrypt_secret_passthrough;
    Alcotest.test_case "encrypt config secrets" `Quick test_encrypt_config_secrets;
