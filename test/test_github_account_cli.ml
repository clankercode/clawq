(** Tests for the redacted GitHub account CLI / agent surface
    (P21.M4.E1.T001).
...
let seed_principal ~db ~id ?(revision = 1) () =
  ...
  in
  ignore (assert_ok (Principal_identity_store.insert_principal ~db ~now:fixed_now p))
...
let test_list_redacts_tokens_and_vault_ids () =
  ...
  Alcotest.(check bool) "no access token in list output" false
    (contains out tokens.access_token);
  Alcotest.(check bool) "no refresh token in list output" false
    (contains out (Option.get tokens.refresh_token));
  Alcotest.(check bool) "vault attachment indicated without raw vault row id" true
    (contains out "vault=attached" || contains out "vault:      attached"
    || contains out "vault:       attached");
  Alcotest.(check bool) "raw vault row id not exported" false
    (contains out vault_id);
  (* private continuation surface is referenced but no URL/code embedded. *)
  Alcotest.(check bool) "no authorization URL" false
    (contains out "github.com/login/oauth/authorize");
  Alcotest.(check bool) "no device user code" false
    (contains out "user_code");
  Alcotest.(check bool) "no client secret" false
    (contains out "client_secret")
...
let test_status_lists_all_when_no_binding () =
  ...
  ignore
    (insert_binding ~db ~principal_id:pa ~id:"bind_two"
       ~github_user_id:2L ~login:(Some "carol") ());
  with_principal_id "prin_a" @@ fun () ->
  let out = Cli.cmd_with_db ~db [ "account"; "status" ] in
  Alcotest.(check bool) "contains both binding ids" true
...
let test_status_refuses_foreign_binding () =
  ...
  Alcotest.(check bool) "cross-principal refused" true
...
let test_use_sets_preference_and_returns_redacted_receipt () =
  ...
  Alcotest.(check bool) "selected confirmation" true
    (contains out "Selected GitHub account bind_use");
  Alcotest.(check bool) "principal_default scope" true
    (contains out "principal_default");
  Alcotest.(check bool) "no token leakage" false
    (contains out "ghu_access");
...
let test_use_refuses_foreign_binding () =
  ...
  Alcotest.(check bool) "refuses foreign use" true
...
let test_link_returns_secret_free_guidance () =
  ...
  Alcotest.(check bool) "mentions private continuation" true
    (contains out "private continuation");
  Alcotest.(check bool) "no URL embedded" false
    (contains out "github.com/login/oauth/authorize");
...
let test_unlink_plan_then_apply_with_digest () =
  ...
  with_principal_id "prin_a" @@ fun () ->
  let plan_out =
    Cli.cmd_with_db ~db [ "account"; "unlink"; "bind_u" ]
  in
  ...
  in
  Alcotest.(check bool) "applied confirmation" true
    (contains apply_out "Unlinked binding bind_u");
  Alcotest.(check bool) "new status in receipt" true
    (contains apply_out "new_status:     unlinked");
  Alcotest.(check bool) "no token leakage" false
    (contains apply_out "ghu_");
...
let test_missing_principal_id_returns_actionable_error () =
  ...
  Fun.protect
    ...
    (fun () ->
      let out = Cli.cmd_with_db ~db [ "account"; "list" ] in
      Alcotest.(check bool) "explains env var" true
...
let test_unknown_subcommand_returns_usage () =
  ...
  Alcotest.(check bool) "mentions delivery module" true
...
let test_min_cli_refuses_account_subcommand () =
  ...
  Alcotest.(check bool) "mentions disabled" true
    (contains out "not available in the minimal build");
  Alcotest.(check bool) "points to full clawq binary" true
    (contains out "full `clawq` binary");
  Alcotest.(check bool) "names the agent tool" true
...
let test_tool_returns_redacted_status_only () =
  ...
  Alcotest.(check bool) "vault_attached flag visible" true
    (contains out "vault=attached");
  Alcotest.(check bool) "no access token in tool output" false
    (contains out tokens.access_token);
  Alcotest.(check bool) "no refresh token in tool output" false
    (contains out (Option.get tokens.refresh_token));
  Alcotest.(check bool) "no authorization URL" false
    (contains out "github.com/login/oauth/authorize");
  Alcotest.(check bool) "no device user code" false
...
let test_tool_single_binding_refuses_foreign_binding () =
  ...
  in
  Alcotest.(check bool) "refuses foreign binding" true
    (contains out "not owned by subject principal")
...
let test_tool_requires_principal_env () =
  ...
  Fun.protect
    ...
    (fun () ->
      let t = ATool.tool ~db in
      let out = Lwt_main.run (t.Tool.invoke (`Assoc [])) in
      Alcotest.(check bool) "explains principal env" true
...
let test_tool_rejects_non_string_binding_id () =
  ...
  let out =
    Lwt_main.run
      (t.Tool.invoke (`Assoc [ ("binding_id", `Int 42) ]))
  in
  Alcotest.(check bool) "rejects non-string" true
