...
let test_process_sse_stream_text () =
  ...
  | Provider.Text { content; model; _ } ->
    ...
    let deltas = List.filter (function Provider.Delta _ -> true | _ -> false) events in
    Alcotest.(check int) "two delta events" 2 (List.length deltas);
    let has_done = List.exists (function Provider.Done -> true | _ -> false) events in
...
let test_process_sse_stream_tool_calls () =
  ...
  let stream = Lwt_stream.of_list chunks in
  let result = Lwt_main.run (Provider.process_sse_stream stream ~on_chunk:(fun _ -> Lwt.return_unit)) in
  match result with
...
let test_provider_config_default_model () =
  let config : Runtime_config.t = {
    ...
    providers = [
      ("test", { api_key = "sk-test"; base_url = Some "http://localhost";
  ...
  Alcotest.(check bool) "contains default_model" true
    (try ignore (Str.search_forward (Str.regexp_string "custom-model") s 0); true
...
let suite =
  [
    ...
    Alcotest.test_case "SSE parse comment line" `Quick test_parse_sse_line_comment;
    Alcotest.test_case "SSE parse non-data line" `Quick test_parse_sse_line_no_prefix;
    Alcotest.test_case "SSE stream text" `Quick test_process_sse_stream_text;
    Alcotest.test_case "SSE stream tool calls" `Quick test_process_sse_stream_tool_calls;
    Alcotest.test_case "SSE stream partial chunks" `Quick test_process_sse_stream_partial_chunks;
    Alcotest.test_case "provider config default_model" `Quick test_provider_config_default_model;
  ]
