...
let test_format_result () =
  ...
  let dirty =
    Worktree_merge.DirtyWorktree { path = "/tmp/wt"; details = "?? file.txt" }
  in
  let s = Worktree_merge.format_result dirty in
  Alcotest.(check bool)
    "dirty worktree error" true
    (contains_substring ~needle:"uncommitted changes" s);
  Alcotest.(check bool)
    "dirty path" true
    (contains_substring ~needle:"/tmp/wt" s);
  Alcotest.(check bool)
    "dirty details" true
    (contains_substring ~needle:"file.txt" s);
  Alcotest.(check bool)
    "commit hint" true
    (contains_substring ~needle:"git commit" s)

let test_finalize_rejects_dirty_worktree () =
  with_temp_git_repo (fun repo_path ->
      let db = Memory.init ~db_path:":memory:" () in
      Background_task.init_schema db;
      let branch = "test-dirty-finalize" in
      let worktree_path = repo_path ^ "-wt" in
      git_cmd repo_path
        (Printf.sprintf "worktree add -b %s %s" branch
           (Filename.quote worktree_path));
      let dirty_file = Filename.concat worktree_path "leftover.txt" in
      let oc = open_out dirty_file in
      output_string oc "uncommitted";
      close_out oc;
      match
        Background_task.enqueue ~db ~runner:Background_task.Codex
          ~automerge:false ~repo_path ~prompt:"test" ~branch ()
      with
      | Error msg -> Alcotest.fail msg
      | Ok id ->
          ignore
          ...
          (match result with
          | Worktree_merge.DirtyWorktree { path; details } ->
              Alcotest.(check string) "path" worktree_path path;
              Alcotest.(check bool)
                "details contain file" true
                (contains_substring ~needle:"leftover.txt" details)
          | other ->
              Alcotest.failf "expected DirtyWorktree, got: %s"
                (Worktree_merge.format_result other));
          let updated_task =
            match Background_task.get_task ~db ~id with
            | Some t -> t
            | None -> Alcotest.failf "task not found after finalize attempt"
          in
          Alcotest.(check (option string))
            "merge_status set to dirty" (Some "dirty") updated_task.merge_status;
          Alcotest.(check bool)
            "worktree still exists" true
            (Sys.file_exists worktree_path);
          git_cmd repo_path
...
let test_try_automerge_rejects_dirty_worktree () =
  with_temp_git_repo (fun repo_path ->
      let db = Memory.init ~db_path:":memory:" () in
      Background_task.init_schema db;
      let branch = "test-dirty-automerge" in
      let worktree_path = repo_path ^ "-wt" in
      git_cmd repo_path
        (Printf.sprintf "worktree add -b %s %s" branch
           (Filename.quote worktree_path));
      let dirty_file = Filename.concat worktree_path "uncommitted.txt" in
      let oc = open_out dirty_file in
      output_string oc "dirty content";
      close_out oc;
      match
        Background_task.enqueue ~db ~runner:Background_task.Codex
          ~automerge:true ~repo_path ~prompt:"test" ~branch ()
      ...
      | Ok id ->
          ...
          | Worktree_merge.DirtyWorktree { path; details } ->
              ...
              Alcotest.(check bool)
                "details contain file" true
                (contains_substring ~needle:"uncommitted.txt" details)
          | other ->
              Alcotest.failf "expected DirtyWorktree, got: %s"
                (Worktree_merge.format_result other));
          let updated_task =
            match Background_task.get_task ~db ~id with
            | Some t -> t
            | None -> Alcotest.failf "task not found after automerge attempt"
          in
          Alcotest.(check (option string))
            "merge_status set to dirty" (Some "dirty") updated_task.merge_status;
          git_cmd repo_path
...
let suite =
  [
    ...
    Alcotest.test_case "finalize rejects dirty worktree" `Quick
      test_finalize_rejects_dirty_worktree;
    Alcotest.test_case "try_automerge rejects dirty worktree" `Quick
      test_try_automerge_rejects_dirty_worktree;
  ]
