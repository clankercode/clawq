...
This guide walks through setting up clawq to receive GitHub webhooks via a Cloudflare named tunnel, so the bot can respond to `/clawq` commands on PRs and issues.
...
| `channels.github.auth_type` | Authentication type, currently only `"pat"` |
| `channels.github.auth_token` | GitHub Personal Access Token |
| `channels.github.repos[].name` | `owner/repo` identifier (informational) |
| `channels.github.repos[].webhook_secret` | Shared secret for HMAC verification |
| `channels.github.repos[].webhook_path` | URL path the webhook POSTs to |
| `channels.github.repos[].allow_users` | List of GitHub usernames, or `["*"]` for all |
| `channels.github.repos[].react_to` | Event types to handle: `pull_request`, `issue_comment`, `pull_request_review_comment`. Empty list means all. |
...
- Check `react_to` in config; an empty list accepts all event types
- The bot only responds to comments/PR bodies containing `/clawq` at the start of a line
