(* User-required typed workflow_dispatch attribution (P21.M3.E3.T007).
   See github_workflow_dispatch_attribution.mli and
...
let field_live_revalidation = "workflow_dispatch_live"
let policy_action = "workflow_dispatch"
...
let policy_action_of_request (_req : Wd.request) = policy_action

let item_key_of_request (req : Wd.request) =
...
let default_live_revalidation : live_revalidation =
  {
    ...
    live_workflow_id = None;
    live_ref = None;
    target_revision = None;
    planned_target_revision = None;
  }

let revalidate_live ~(req : Wd.request) ~live =
  if live.already_applied then
    Error
      "duplicate or replay: this workflow_dispatch was already applied; refuse \
       silent re-dispatch"
  else if not live.repo_present then
    Error
      "target repository is missing or no longer addressable; re-resolve the \
       repo and re-preview"
  else if not live.workflow_present then
    Error
      "target workflow is missing or no longer addressable; re-resolve the \
       workflow id/file and re-preview"
  else if not live.ref_present then
    Error
      "target ref is missing or no longer addressable; re-resolve the branch, \
       tag, or SHA and re-preview"
  else if not live.inputs_still_valid then
    Error
      "resolved workflow inputs are no longer valid against the allowed \
       schema; re-select inputs and re-preview"
  else if String.trim req.repo_full_name = "" then
    Error "workflow_dispatch repo_full_name must be non-empty"
  else if String.trim req.workflow_id = "" then
    Error "workflow_dispatch workflow_id must be non-empty"
  else if String.trim req.ref_ = "" then
    Error "workflow_dispatch ref must be non-empty"
  else
    let planned_wf = String.trim req.workflow_id in
    let planned_ref = String.trim req.ref_ in
    match live.live_workflow_id with
    | Some current
      when not
             (String.equal planned_wf (String.trim current)
             || String.trim current = "") ->
        Error
          (Printf.sprintf
             "stale workflow_id: planned %s live %s; revalidate the workflow \
              identity before dispatch"
             planned_wf current)
    | _ -> (
        match live.live_ref with
        | Some current
          when not
                 (String.equal planned_ref (String.trim current)
                 || String.trim current = "") ->
            Error
              (Printf.sprintf
                 "stale ref: planned %s live %s; revalidate the git ref before \
                  dispatch"
                 planned_ref current)
        | _ -> (
            ...
            | Some planned, Some current
              when not
                ...
                Error
                  (Printf.sprintf
                     "stale target: planned revision %s live %s; revalidate \
...
let authorize_capability ~req ~route ~pilot ~user_auth_available
    ?(now = Unix.gettimeofday ()) () =
  Wd.authorize ~route ~pilot ~user_auth_available ~req ~now ()
...
let merge_live_into_auth (auth : Auth.request) ~req ~live : Auth.request =
...
let authorize_preview ~db ~req ~route ~pilot ~user_auth_available ~auth ~live
    ?item_key ?room_id ?plan_id ?actor_snapshot ?github_user_id
    ?(now = Unix.gettimeofday ()) () =
  let policy_action = policy_action_of_request req in
  let item_key =
    match item_key with
    | Some k when String.trim k <> "" -> Some (String.trim k)
    | _ -> item_key_of_request req
  in
  match
    authorize_capability ~req ~route ~pilot ~user_auth_available ~now ()
  with
  | Error reason ->
      let audit =
        match
      ...
      | Ok () -> (
          ...
          | Auth.Allow allow -> (
              (* Production User_required path: never accept App as resolved
                 mode. P19 pilot App uses Wd.authorize without this path. *)
...
let dispatch ~db ~req ~live_auth ~prior ~live ?vault_id ?expected ?item_key
          ...
          | Ok () -> (
              ...
              let reason =
                Printf.sprintf
                  ...
                  (String.trim req.workflow_id)
                  (String.trim req.ref_)
              in
              let result =
  ...
  `Assoc
    (sort_assoc
       [
         ...
         ("workflow_present", `Bool l.workflow_present);
         ("ref_present", `Bool l.ref_present);
         ("inputs_still_valid", `Bool l.inputs_still_valid);
         ("already_applied", `Bool l.already_applied);
         opt_string "live_workflow_id" l.live_workflow_id;
         opt_string "live_ref" l.live_ref;
            ...
            message =
              Printf.sprintf
                "Staged %s attribution mode=%s used_app_fallback=%b; \
                 revalidate workflow/ref/inputs and issue opaque lease at \
...
let is_workflow_dispatch_attribution_plan (plan : Setup_plan.t) =
  match plan.apply_payload.kind with
  | Setup_plan.Generic "github_workflow_dispatch" -> true
  | _ -> false

let has_attribution_allow (plan : Setup_plan.t) =
  is_workflow_dispatch_attribution_plan plan
  &&
...
let plan_with_attribution ~db ~principal ~room_id ~req ~base_revision ~auth
    ~live ~route ~pilot ~user_auth_available ?actor_snapshot ?github_user_id
    ?(now = Unix.gettimeofday ()) () =
  match
    authorize_preview ~db ~req ~route ~pilot ~user_auth_available ~auth ~live
  ...
  | Ok preview -> (
      let plan_res =
        match route with
        | Some r ->
            Wd.plan_dispatch ~db ~principal ~room_id ~pilot ~user_auth_available
              ~req ~base_revision ~route:r ~now ()
        | None ->
            Wd.plan_dispatch ~db ~principal ~room_id ~pilot ~user_auth_available
              ~req ~base_revision ~now ()
      in
      match plan_res with
      | Error e -> Error e
      | Ok plan -> (
          ...
          let _ =
            Audit.record_authorize_decision ~db ~decision:preview.decision
              ~kind:Audit.Audit ?item_key:(item_key_of_request req) ~room_id
              ~plan_id:plan.id ?actor_snapshot ?github_user_id ~now ()
          in
          match Setup_plan_apply.replace_pending_plan ~db plan with
          | Error e -> Error e
          | Ok () -> Ok { plan; preview }))

let request_of_plan (plan : Setup_plan.t) : (Wd.request, string) result =
  ...
  | Setup_plan.Generic "github_workflow_dispatch" ->
      let repo =
        ...
        | Some s -> s
        | None -> Option.value (get_string "repo_full_name" data) ~default:""
      in
      let workflow_id =
        match field "workflow_id" with
        | Some s -> s
        | None -> Option.value (get_string "workflow_id" data) ~default:""
      in
      let ref_ =
        match field "ref" with
        | Some s -> s
        | None -> Option.value (get_string "ref" data) ~default:""
      in
      let inputs =
        match action_fields with
        | Some fields -> inputs_of (List.assoc_opt "inputs" fields)
        | None -> inputs_of (member_opt "inputs" data)
      in
      let item_key =
      ...
      Ok
        {
          Wd.repo_full_name = repo;
          workflow_id;
          ref_;
          inputs;
          item_key;
          allowed_input_names = None;
        }
  | Setup_plan.Generic other ->
      Error
        (Printf.sprintf
           "plan kind %S is not a workflow_dispatch attribution plan" other)
  | _ -> Error "plan is not a generic workflow_dispatch action"
...
let prepare_dispatch_from_plan ~db ~plan ~live_auth ~live ?vault_id ?expected
  ...
  | Ok None ->
      Error
        "plan has no staged attribution_allow; cannot prepare \
         workflow_dispatch dispatch"
  | Ok (Some prior) -> (
      match request_of_plan plan with
      | Error e -> Error e
      | Ok req -> (
          match
            dispatch ~db ~req ~live_auth ~prior ~live ?vault_id ?expected
              ?item_key:(item_key_of_request req)
