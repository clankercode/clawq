...
include Runtime_config_models
include Runtime_config_defaults
...
let resolve_effective_access (cfg : t) ~session_key ?room_profile () :
  ...
  let instruction_text_items = merge_effective_items instruction_text_items in
  {
    ...
    egress_rules =
      (* For egress rules, higher-priority scopes (Room > Channel >
         Workspace > Default) should come first since first match wins.
