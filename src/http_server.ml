            ...
            if session_id = "" then bad_request "session_id is required"
              ...
              if not sess_ok then rate_limit_response ()
                    ...
                    let response =
                      ...
                      | Some db ->
                          Slash_commands.format_cron
                            ~connector:Format_adapter.Plain ~db ~session_key:key
                            action
                ...
                | Slash_commands.Model
                    (Slash_commands.ModelList (provider, availability)) ->
                    let response =
                            ...
                            | None ->
                                ...
                                let hint =
                                  ...
                                  | Models_catalog.Legacy ->
                                      Printf.sprintf
                                        "\n\
                                         Hint: use %s:%s format instead of \
                                         %s/%s."
                                      ...
                                      if not provider_in_config then
                                        Printf.sprintf
                                          "\n\
                                           ...
                                           config. Add it to your config.json \
                                           to use this model."
                                          provider
                                      else ""
                        ...
                        | None -> (
                            ...
                            | Ok () ->
                                let msg =
                                  ...
                                  | Models_catalog.Legacy ->
                                      Printf.sprintf
                                        "Default model set to: %s (provider: \
                                         %s)%s\n\
