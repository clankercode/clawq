...
let status_cmd =
  simple "status" "Show runtime configuration and daemon status."
...
let mcp_cmd =
  simple "mcp"
    "Start the MCP server (exposes configured tools over the Model Context Protocol)."
...
let cron_cmd =
  with_args "cron" "Manage cron jobs for scheduled agent messages."
    ...
    ; `I ("add NAME SESSION SCHEDULE MSG",
          "Add a cron job. SCHEDULE is a cron expression (e.g. \"0 9 * * 1-5\").")
...
let audit_cmd =
  with_args "audit" "View and manage the security audit log."
    ...
    ; `I ("purge",
          "Delete old entries per the retention policy in config.")
    ]
...
let runtime_cmd =
  with_args "runtime"
    ...
    ; `I ("native start|stop|health", "Control or health-check the native runtime.")
    ; `I ("docker start|stop|health", "Control or health-check the Docker runtime.")
    ]
    ...
    ~man:
      ...
      ; `P
          "clawq is a modular AI assistant runtime with an agent loop, \
           ...
           an HTTP gateway, cron scheduling, audit logging, and MCP server."
