...
external add_rule_path_c : int -> string -> int -> unit
  = "caml_landlock_add_rule_path"
...
let add_path_safe ruleset_fd path access =
  try
    if Sys.file_exists path then add_rule_path_c ruleset_fd path access
    else Logs.debug (fun m -> m "Landlock: skipping non-existent path %s" path)
  with exn ->
    Logs.warn (fun m ->
...
let sandbox_workspace ~(config : Runtime_config.t) =
  if not (available ()) then begin
    ...
    with exn ->
      Logs.err (fun m ->
          m
            "Landlock sandbox activation failed: %s (continuing without \
             sandbox)"
            (Printexc.to_string exn))
