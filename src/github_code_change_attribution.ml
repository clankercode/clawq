(* User-required code work and constrained PR creation attribution
   (P21.M3.E3.T008). See github_code_change_attribution.mli and
...
let field_live_revalidation = "code_change_live"
let policy_action = "code_change"
...
let policy_action_of_family (_f : family) = policy_action
...
let revalidate_code_work_result ~live ~now =
  match live.code_work_result_status with
  | None -> Ok ()
  | Some status ->
      Code.check_code_work_result_usable ~result_status:status
        ?finished_at:live.code_work_finished_at
        ?max_age_seconds:live.max_age_seconds ~now ()

let revalidate_live ~family ~live =
  let now =
    match live.max_age_seconds with
    | Some _ -> Unix.gettimeofday ()
    | None -> Unix.gettimeofday ()
  in
  if live.already_applied then
    Error
      "duplicate or replay: this code-change / PR-create was already applied; \
       refuse silent re-dispatch"
  else if not live.repo_present then
    Error
      "target repository is missing or no longer addressable; re-resolve the \
       repo and re-preview"
  else if not live.base_present then
    Error
      "base branch is missing or no longer addressable; re-resolve the base \
       and re-preview"
  else
      ...
      when not
        ...
        Error
          (Printf.sprintf
             ...
             planned current)
    | _ -> (
        match revalidate_code_work_result ~live ~now with
        | Error e -> Error e
        | Ok () -> (
            match family with
            | Code_work _ -> Ok ()
            | Pr_create req -> (
                (* Confirmed_code_work head: re-check status when live status
                   not separately supplied (already checked when present). *)
                  (match req.head with
                  | Code.Explicit_branch _ -> Ok ()
                  | Code.Confirmed_code_work { status; finished_at; _ } ->
                      if Option.is_some live.code_work_result_status then Ok ()
                      else
                        Code.check_code_work_result_usable ~result_status:status
                          ?finished_at ?max_age_seconds:live.max_age_seconds
                          ~now ())
                |> function
                | Error e -> Error e
                | Ok () -> (
                    match live.current_refs with
                    | None -> Ok ()
                    | Some current ->
                        let planned_head =
                          match req.head with
                          | Code.Explicit_branch b -> String.trim b
                          | Code.Confirmed_code_work { head_branch; _ } ->
                              String.trim head_branch
                        in
                        let planned_base = String.trim req.base_branch in
                        let planned_sha =
                          match req.head_sha with
                          | Some s when String.trim s <> "" -> Some s
                          | _ -> (
                              match req.head with
                              | Code.Confirmed_code_work { head_sha; _ }
                                when String.trim head_sha <> "" ->
                                  Some head_sha
                              | _ -> None)
                        in
                        Code.revalidate_pr_refs ~planned_head ~planned_base
  ...
  | Ok () ->
      {
        ...
        revision =
          ...
          | _ -> (
              match live.current_refs with
              | Some r when String.trim r.head_sha <> "" ->
                  Some (String.trim r.head_sha)
  ...
  | Error detail ->
      {
        ok = false;
        revision =
          (match live.planned_target_revision with
          | Some _ as r -> r
          | None -> (
              match live.current_refs with
              | Some r when String.trim r.head_sha <> "" ->
...
let authorize_capability ~family ~route ~pilot ~user_auth_available
  ...
  match family with
  | Code_work req ->
      Code.authorize_code_work ~route ~pilot ~user_auth_available ~req ~now ()
  | Pr_create req ->
      Code.authorize_pr_create ~route ~pilot ~user_auth_available ~req ~now ()
      ...
      | Ok () -> (
          ...
          | Auth.Allow allow -> (
              (* Production User_required path: never accept App as resolved
                 mode. P19 pilot App uses Code.authorize_* without this path. *)
  ...
  | Ok () -> (
      ...
      | Ok issued -> (
          ...
          | Ok () -> (
              ...
              let reason =
                Printf.sprintf
                  "dispatch ok action=%s family=%s mode=%s user_lease=%b \
                   item=%s"
                  policy_action (family_label family) mode_s has_user_lease
...
let live_to_json (l : live_revalidation) =
  let refs_json = function
    | None -> `Null
    | Some (r : Code.live_refs) ->
        `Assoc
          (sort_assoc
             [
               ...
               opt_string "base_sha" r.base_sha;
               ("head_exists", `Bool r.head_exists);
               ("base_exists", `Bool r.base_exists);
             ])
  in
  `Assoc
    (sort_assoc
       [
         ...
         ("base_present", `Bool l.base_present);
         ("already_applied", `Bool l.already_applied);
         ("current_refs", refs_json l.current_refs);
         ( "code_work_result_status",
           match l.code_work_result_status with
           | None -> `Null
           | Some s -> `String (Code.result_status_to_string s) );
         opt_string "code_work_finished_at" l.code_work_finished_at;
         ( "max_age_seconds",
           match l.max_age_seconds with None -> `Null | Some n -> `Float n );
            ...
            message =
              Printf.sprintf
                "Staged %s attribution mode=%s used_app_fallback=%b; \
                 revalidate live refs/result and issue opaque lease at \
                 apply/dispatch. No raw token on plan (runner/shell/Git \
                 transport never receive personal token)."
...
let is_code_change_attribution_plan (plan : Setup_plan.t) =
  Code.is_code_change_plan plan

let has_attribution_allow (plan : Setup_plan.t) =
  is_code_change_attribution_plan plan
  &&
...
let plan_with_attribution ~db ~principal ~room_id ~family ~base_revision ~auth
  ...
  | Ok preview -> (
      let plan_res =
        ...
        | Code_work req, Some r ->
            Code.plan_code_work ~db ~principal ~room_id ~pilot
              ~user_auth_available ~req ~base_revision ~route:r ~now ()
        | Code_work req, None ->
            Code.plan_code_work ~db ~principal ~room_id ~pilot
              ~user_auth_available ~req ~base_revision ~now ()
        | Pr_create req, Some r ->
            Code.plan_pr_create ~db ~principal ~room_id ~pilot
              ~user_auth_available ~req ~base_revision ~route:r ~now ()
        | Pr_create req, None ->
            Code.plan_pr_create ~db ~principal ~room_id ~pilot
              ~user_auth_available ~req ~base_revision ~now ()
...
let head_source_of_json json : (Code.head_source, string) result =
  match json with
  | `Assoc _ as j -> (
      match get_string "kind" j with
      | Some "explicit_branch" -> (
          match get_string "head_branch" j with
          | None -> Error "explicit_branch head_source missing head_branch"
          | Some b -> Ok (Code.Explicit_branch b))
      | Some "confirmed_code_work" -> (
          let plan_id =
            Option.value (get_string "code_work_plan_id" j) ~default:""
          in
          let head_branch =
            Option.value (get_string "head_branch" j) ~default:""
          in
          let head_sha = Option.value (get_string "head_sha" j) ~default:"" in
          let status_s =
            Option.value (get_string "status" j) ~default:"succeeded"
          in
          match Code.result_status_of_string status_s with
          | Error e -> Error e
          | Ok status ->
              Ok
                (Code.Confirmed_code_work
                   {
                     code_work_plan_id = plan_id;
                     head_branch;
                     head_sha;
                     status;
                     finished_at = get_string "finished_at" j;
                   }))
      | Some other ->
          Error (Printf.sprintf "unknown head_source kind: %s" other)
      | None -> Error "head_source.kind missing")
  | _ -> Error "head_source must be a JSON object"
...
let family_of_plan (plan : Setup_plan.t) : (family, string) result =
  ...
  let action_fields =
    ...
    | Some (`Assoc fields) -> Some (`Assoc fields)
    | Some other -> Some other
    | None -> None
  in
  let field_from_action name =
  ...
  in
  let str name =
    match field_from_action name with
    | Some s -> s
    | None -> Option.value (get_string name data) ~default:""
  in
  match plan.apply_payload.kind with
  | Setup_plan.Generic "github_code_work" ->
      let req : Code.code_work_request =
        {
          ...
          branch_prefix =
            (let p = str "branch_prefix" in
             if p = "" then Code.default_branch_prefix else p);
          head_branch =
            (match field_from_action "head_branch" with
            | Some s -> Some s
            | None -> get_string "head_branch" data);
          item_key =
            (match field_from_action "item_key" with
            | Some s -> Some s
            | None -> get_string "item_key" data);
          related_issue = None;
        }
      in
      Ok (Code_work req)
  | Setup_plan.Generic "github_pr_create" -> (
      let head_source_json =
        match action_fields with
        | Some (`Assoc fields) -> List.assoc_opt "head_source" fields
        | _ -> member_opt "head_source" data
      in
      let head_res =
        match head_source_json with
        | Some j -> head_source_of_json j
        | None ->
            let head = str "head_branch" in
            if head = "" then Error "pr_create plan missing head_branch"
            else Ok (Code.Explicit_branch head)
      in
      match head_res with
      | Error e -> Error e
      | Ok head ->
          let req : Code.pr_create_request =
            {
              repo_full_name = str "repo_full_name";
              base_branch = str "base_branch";
              title = str "title";
              body =
                (match field_from_action "body" with
                | Some s -> Some s
                | None -> get_string "body" data);
              draft =
                (match member_opt "draft" data with
                | Some (`Bool b) -> b
                | _ -> (
                    match action_fields with
                    | Some (`Assoc fields) -> (
                        match List.assoc_opt "draft" fields with
                        | Some (`Bool b) -> b
                        | _ -> false)
                    | _ -> false));
              head;
              branch_prefix =
                (let p = str "branch_prefix" in
                 if p = "" then Code.default_branch_prefix else p);
              head_sha =
                (match field_from_action "head_sha" with
                | Some s -> Some s
                | None -> get_string "head_sha" data);
              item_key =
                (match field_from_action "item_key" with
                | Some s -> Some s
                | None -> get_string "item_key" data);
  ...
  | Setup_plan.Generic other ->
      Error
        (Printf.sprintf "plan kind %S is not a code-change attribution plan"
           other)
  | _ -> Error "plan is not a generic code-change action"
...
let prepare_dispatch_from_plan ~db ~plan ~live_auth ~live ?vault_id ?expected
  ...
  | Ok None ->
      Error
        "plan has no staged attribution_allow; cannot prepare code-change \
         dispatch"
  | Ok (Some prior) -> (
