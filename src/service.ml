  ...
  | None ->
    ...
    | _pid ->
      ...
      (match wait_for_ready 30 with
       ...
       | None ->
         Printf.sprintf
           "Daemon failed to become ready. Check logs at %s"
           (log_path ()))
...
let cmd_stop () =
  ...
  | Some pid ->
    if not (pid_identity_ok pid) then begin
      remove_pid ();
      Printf.sprintf "Refusing to stop pid %d: process identity mismatch; stale pid state removed" pid
    end else begin
