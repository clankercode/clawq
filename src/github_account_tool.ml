(** Narrow agent tool: redacted GitHub account status only (P21.M4.E1.T001).
    ...
    Returns redacted account + preference inspection for the current
    Principal. Never exposes vault tokens, sealed ciphertext, vault row ids,
    authorization URLs, device codes, or callback errors. Authorization
    continuations and lifecycle mutations are out of scope for agents; they
    happen through the CLI (or the configured Connector / app) and are
    delivered privately via {!Github_user_auth_delivery}.
...
let current_principal_id () =
  ...
  | Some raw ->
      ...
      if trimmed = "" then
        Error
          (Printf.sprintf
             "github_account tool: %s must be set to a non-empty Principal \
              id."
             principal_env_var)
      else (
...
let render_inspect (inspect : Surf.account_inspect) =
  ...
  Buffer.add_string buf
    (Printf.sprintf "  principal: %s\n  kind:      %s\n"
       inspect.principal_id inspect.surface_kind);
  Buffer.add_string buf
    (Printf.sprintf "  accounts:  %d\n" (List.length inspect.accounts));
  List.iter
    (fun (a : Surf.redacted_account) ->
      Buffer.add_string buf
        (Printf.sprintf "    - %s (%s) host=%s app=%d user=%Ld status=%s \
                         vault=%s lineage=%s revision=%d\n"
           a.binding_id
           (Option.value ~default:"<unknown>" a.login)
           a.host a.app_id a.github_user_id
...
let tool ~(db : Sqlite3.db) : Tool.t =
              ...
              ( "binding_id",
                `Assoc
                  [
                    ...
                    ( "description",
                      `String
                        "Optional binding id to inspect a single binding \
                         (with historical snapshots). Omit for the current \
  ...
  let param_err detail =
    Tool.make_param_error ~tool_name:"github_account"
  ...
  in
  {
    ...
    description =
      "Inspect the current Principal's redacted GitHub account status. \
       Returns binding ids, logins, host/App/numeric GitHub user id, \
       authorization status (Pending/Authorized/Disabled/Revoked/Unlinked), \
       vault-attached flag (presence only), revision, lineage id, and stored \
       Principal preferences. Never exposes vault tokens, sealed ciphertext, \
       vault row ids, authorization URLs, device codes, or callback errors. \
       Use this when an agent needs to know which GitHub account the current \
       Principal is using; for lifecycle changes (link, relink, unlink) run \
       the matching `clawq github account ...` CLI command.";
...
let disabled_tool : Tool.t =
  ...
  in
  {
    Tool.name = "github_account";
    description =
      "Disabled in the minimal build. Use the full `clawq` binary for \
       redacted GitHub account status.";
    parameters_schema = schema;
    invoke =
      (fun ?context:_ _args ->
        Lwt.return
