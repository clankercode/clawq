...
let cmd_status () =
  ...
  add (Printf.sprintf "  cli channel: %s" (if cfg.channels.cli then "enabled" else "disabled"));
    ...
    List.iter
      (fun (name, (p : Runtime_config.provider_config)) ->
        if Runtime_config.is_key_set p.api_key
          ...
          add
            (Printf.sprintf
               "WARNING: Provider '%s' has plaintext API key but encrypt_secrets is enabled. Use \"$ENV_VAR\" syntax to reference environment variables."
               name))
      cfg.providers;
...
let cmd_onboard () =
  ...
  if Sys.file_exists config_path then
    ...
    (try if not (Sys.file_exists config_dir) then Sys.mkdir config_dir 0o755 with _ -> ());
    let template =
      {|{
  ...
  "security": {
    ...
    (try
       ...
       close_out oc
     with exn ->
       failwith (Printf.sprintf "Failed to write config: %s" (Printexc.to_string exn)));
...
let cmd_models () =
  ...
  | providers ->
    let lines =
      List.map
        (fun (name, (p : Runtime_config.provider_config)) ->
          let url = match p.base_url with Some u -> u | None -> "(default)" in
          let model_info = match p.default_model with Some m -> Printf.sprintf " model: %s" m | None -> "" in
    ...
    ^ Printf.sprintf "\nDefault provider: %s" (match cfg.default_provider with Some p -> p | None -> "(auto)")
...
let cmd_memory () =
  let cfg = get_config () in
  Printf.sprintf "Memory backend: %s\nSearch enabled: %b" cfg.memory.backend cfg.memory.search_enabled
...
let cmd_capabilities () =
  ...
  add
    (Printf.sprintf "  - LLM chat: %d provider(s) configured (%s)"
       ...
       (if active_providers = [] then "none active" else String.concat ", " (List.map fst active_providers)));
  if cfg.channels.cli then add "  - CLI channel: enabled";
  add (Printf.sprintf "  - Memory: %s (FTS search: %s)" cfg.memory.backend
  ...
  if cfg.security.tools_enabled then begin
    ...
    add (Printf.sprintf "  - Tools: %d registered (%s)" (List.length tool_names) (String.concat ", " tool_names))
  end else
    add "  - Tools: disabled";
...
let cmd_auth args =
  ...
  | [ "encrypt" ] ->
    if not (get_config ()).security.encrypt_secrets then
      ...
      | Ok key ->
        let home = try Sys.getenv "HOME" with Not_found -> "/tmp" in
        let config_path = Filename.concat (Filename.concat home ".clawq") "config.json" in
        if not (Sys.file_exists config_path) then
          ...
          in
          match json with
          | Error exn -> Printf.sprintf "Failed to read config: %s" (Printexc.to_string exn)
          | Ok json ->
            (match Secret_store.encrypt_config_secrets ~key json with
             ...
             | Ok new_json ->
               (try
                ...
                with exn -> Printf.sprintf "Failed to write config: %s" (Printexc.to_string exn)))
    end
  | _ ->
    let cfg = get_config () in
    match cfg.providers with
        ...
        List.map
          (fun (name, (p : Runtime_config.provider_config)) ->
            Printf.sprintf "  %s: %s" name
              (if Runtime_config.is_key_set p.api_key then redact_key p.api_key else "not set"))
          providers
      in
      "API key status:\n" ^ String.concat "\n" lines
...
let get_db () =
  ...
  (try if not (Sys.file_exists clawq_dir) then Sys.mkdir clawq_dir 0o755 with _ -> ());
...
let cmd_cron args =
  ...
  | [ "remove"; name ] ->
    let db = get_db () in
    Scheduler.init_schema db;
    if Scheduler.remove_job ~db ~name then Printf.sprintf "Removed job '%s'" name
    ...
    | [ "list" ] | [] ->
      ...
      if rows = [] then "No audit log entries."
        ...
        let lines =
          List.map
            (fun (r : Audit.row) ->
              Printf.sprintf "  %-5d %-20s %-18s %-10s %s" r.id r.timestamp r.event_type
                (match r.tool_name with Some n -> n | None -> "")
                (match r.details with
                 | Some d when String.length d > 50 -> String.sub d 0 50 ^ "..."
                 | Some d -> d
                 | None -> ""))
            rows
        in
...
let unsupported cmd =
  Printf.sprintf "%s is disabled in minimal build. Use full 'clawq' binary for server/network integrations." cmd
