      ...
      Error
        (Printf.sprintf
           "Error: %s is required so `clawq github account ...` commands act \
            on the current Principal. Set %s=<principal-id> (see `clawq \
            principal list`)."
...
let format_account_line (a : Surf.redacted_account) =
  Printf.sprintf
    "- %s (binding %s)\n\
    \    host=%s app=%d user=%Ld login=%s status=%s revision=%d vault=%s \
     lineage=%s"
...
let format_preference_line (p : Surf.redacted_preference) =
  Printf.sprintf "  %s binding=%s lineage=%s (rev=%d, updated=%s)" p.scope
...
let render_single_account ~principal_id (a : Surf.redacted_account)
  ...
  Buffer.add_string buf
    (Printf.sprintf "GitHub account %s (principal %s)\n" a.binding_id
       principal_id);
  Buffer.add_string buf (Printf.sprintf "  lineage:    %s\n" a.lineage_id);
  Buffer.add_string buf (Printf.sprintf "  host/app:   %s/%d\n" a.host a.app_id);
  Buffer.add_string buf (Printf.sprintf "  user_id:    %Ld\n" a.github_user_id);
  Buffer.add_string buf
    (Printf.sprintf "  login:      %s\n"
       (Option.value ~default:"<unknown>" a.login));
  Buffer.add_string buf
    (Printf.sprintf "  status:     %s\n" a.authorization_status);
  Buffer.add_string buf (Printf.sprintf "  revision:   %d\n" a.revision);
  Buffer.add_string buf
    (Printf.sprintf "  vault:      %s\n"
       (if a.vault_attached then "attached" else "none"));
  Buffer.add_string buf (Printf.sprintf "  created:    %s\n" a.created_at);
  Buffer.add_string buf (Printf.sprintf "  updated:    %s\n" a.updated_at);
  if snaps = [] then Buffer.add_string buf "  snapshots:  0\n"
    ...
    List.iter
      (fun (s : Surf.redacted_snapshot) ->
        Buffer.add_string buf
          (Printf.sprintf "    - %s @ %s (%s) prior_status=%s prior_login=%s\n"
...
let cmd_status ~db ~principal_id binding_id =
  ...
  | Ok surface -> (
      ...
      | Some id -> (
          match (non_empty_trim "binding_id" id : (string, string) result) with
          | Error e -> "Error: " ^ e
          | Ok trimmed -> (
              match
      ...
      | Ok surface -> begin
          ...
          | Ok (Some binding) ->
              ...
              then
                ...
                match
                  Pref.make_preference_value ~binding_id:binding.id
                ...
                | Ok value ->
                    ...
                    | Ok stored -> (
                        ...
                        | None -> "Error: preference stored without binding id")
                    end
              end
        end
    end
...
let link_guidance subcommand =
  Printf.sprintf
    "GitHub account %s is delivered through the configured Connector's private \
     continuation. The authorization URL, device codes, and callback results \
     are routed only to authenticated private channels (Connector DM, \
              ...
              | Ok plan -> (
                  if not (String.equal plan.digest digest) then
                    ...
                    | Surf.Refused { reason; conflicts } ->
                        Printf.sprintf "Error: unlink refused: %s%s" reason
                          (if conflicts = [] then ""
                           else
                             "\n  conflicts:\n"
                             ^ String.concat ""
                    ...
                    | Surf.Stale_revision msg ->
                        Printf.sprintf
                          "Error: binding revision conflict: %s. Re-run `clawq \
                           github account unlink %s` to refresh the plan."
                          msg id))
          | None -> (
              match
                Surf.plan_account_action ~db ~surface ~kind:Surf.Unlink_account
...
let usage () =
  Printf.sprintf
    "Usage: clawq github account <subcommand> [args]\n\n\
     ...
     current Principal.\n\
     status [BINDING_ID]        Status of every binding, or one binding with \
     ...
     errors, and account-control payloads are delivered privately via %s and \
     never appear in this CLI output."
...
let cmd_with_db ~db args =
  let rec resolve_principal = function
    ...
    | "link" :: [] -> cmd_link ()
    | "link" :: _ -> "Error: `clawq github account link` takes no arguments"
    | [ "relink"; binding_id ] -> (
        match cli_principal_id () with
        | Error e -> e
    ...
    | _ -> usage ()
  in
  match args with "account" :: rest -> resolve_principal rest | _ -> usage ()
