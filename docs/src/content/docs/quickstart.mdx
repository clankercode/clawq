---
title: "Quick Start"
description: "Install and run clawq in 5 minutes"
section: "preface"
order: 2
---
...
The current npm release artifact is built on Linux. If you are on another platform, use the [Development Guide](/development) source build path for now.
...
If you want to build or contribute from source instead, use the [Development Guide](/development). Source builds need opam, OCaml 5.1, and system SQLite headers.
...
To route through the OpenCodex LAN proxy instead, use:
...
Clawq reads the required `ocx_` token from a configured `api_key`, `OPENCODEX_API_AUTH_TOKEN`, or `~/.opencodex/api-token`, in that order. If `cachy.lan` does not resolve, use `http://10.100.1.2:10100/v1`. OpenCodex always uses Responses HTTP; it does not use ChatGPT OAuth or websockets.
...
<Callout type="tip" title="Interrupt a stuck turn">
  ...
  Admins can also send a bare <code>stop</code> or <code>/stop</code> while a
  session is busy to terminate the active turn cleanly with
  <code>Stopped by admin.</code> and without enqueueing a follow-up message.
  Use <code>!stop</code> when you want the older interrupt-plus-follow-up
  behavior.

  In Telegram, Discord, and Slack, a salute reaction means Clawq acknowledged
  the interrupt while the session was busy.

  To send a non-interrupting note after the current turn finishes, use
  <code>/followup MESSAGE</code>. Use <code>/followup-append MESSAGE</code> to
  extend the latest queued follow-up.
</Callout>
...
- For an OpenCodex 401, verify the configured `api_key`, `OPENCODEX_API_AUTH_TOKEN`, then `~/.opencodex/api-token` contains a valid `ocx_` token. If the host cannot be resolved, use the `10.100.1.2` base URL.
...
- **[Configuration Reference](/configuration)** — all config sections and fields
- **[CLI Reference](/cli-reference)** — complete command reference with slash commands
- **[Channels](/channels)** — detailed setup guides for Discord, Slack, GitHub, Teams, and more
