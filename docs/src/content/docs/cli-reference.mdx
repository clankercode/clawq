...
| `clawq session list` | List active sessions with state, channel, and pending queue counts |
| `clawq session show KEY` | Show session details and message history |
| `clawq session inject SESSION MSG` | Inject a message into a session's durable inbound queue |
...
| `/cron [list\|show\|add\|edit\|remove\|history]` | Manage cron jobs: list, show details, create, edit schedule/message, remove, view run history |
