...
`clawq plan start` is a separate workflow from `clawq delegate`. It runs a multi-stage planner/reviewer/coder loop in the foreground and blocks until that pipeline finishes. A delegated agent may choose to invoke it inside its own worktree when an explicit plan-first pass is warranted, but there is no automatic delegate-to-plan handoff.
