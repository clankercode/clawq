...
| `clawq notify --channel telegram\|teams --target TARGET [--account NAME] [--parse-mode MODE] MESSAGE...` | Send one connector message without starting an agent turn |
| `clawq notify --channel telegram\|teams --list-targets [--account NAME]` | List target IDs previously recorded in the local Clawq database |
...
| `clawq github route inspect ROUTE_ID` | Inspect a persisted GitHub Item/Repo/Org route (full build) |
| `clawq github route list ROOM` | List GitHub routes for a Room (full build) |
| `clawq github route preview ROOM --envelope-json JSON` | Read-only filter explain for a safe normalized GitHub envelope |
| `CLAWQ_ADMIN=1 clawq github route diagnostics [--room ROOM] [--json]` | Collect redacted local route/App/delivery/catalog diagnostics; `--room ROOM --envelope-json JSON [--json]` adds safe non-mutating explain fields |
| `CLAWQ_ADMIN=1 clawq github route export [--room ROOM]` | Emit the redacted JSON diagnostics export; `--room ROOM --envelope-json JSON` adds safe explain fields |
| `CLAWQ_ADMIN=1 clawq github route validate [--room ROOM] [--json]` | Validate local upgrade state and documentation drift; only a Room-effective frozen catalog can pass, otherwise `catalog_state_unavailable` warns |
| `clawq github route plan ROOM SELECTOR [--filter-json JSON]` | Create a confirmable typed route plan; requires admin and principal identity |
| `clawq github route change ROUTE_ID [--filter-json JSON]` | Plan a typed filter/mode/enabled update; requires admin and principal identity |
| `clawq github route disable\|remove\|apply` | Plan disable/remove or explicitly apply a confirmed route plan |
| `clawq github app deliveries\|apply` | Inspect or explicitly apply GitHub App setup delivery plans |
| `clawq github diagnostics route\|audit` | Read-only route readiness and audit information |
...
| `clawq memory forget KEY` | Delete a memory entry by key. In interactive sessions, `memory_forget` also archives forgotten content under the session workspace as `memory_archive.jsonl` |
...
| `clawq rooms` | Manage room profiles, bindings, workspaces, and the room activity ledger |
| `clawq rooms list` | List all room profiles and bindings |
| `clawq rooms show ROOM_ID` | Show a room binding and profile details |
| `clawq rooms workspace ROOM_ID` | Show/create the preserved workspace path for a room |
| `clawq rooms memory list ROOM_ID` | List room-scoped memories when the room has list access |
| `clawq rooms memory show ROOM_ID MEMORY_ID` | Show a room-scoped memory when the room has read access |
| `clawq rooms memory save ROOM_ID REFERENCE CONTENT...` | Save or update a room-scoped memory when the room has write access; creates the first scope for bound rooms or admins |
| `clawq rooms ledger list [FILTERS]` | List room activity ledger entries (admin-only). Filters: `--room-id`, `--event-type`, `--from`, `--to`, `--actor`, `--profile-id`, `--thread-id`, `--task-id`, `--background-id`, `--requester`, `--status` |
| `clawq rooms ledger export [FILTERS] [--format json\|jsonl]` | Export filtered room activity ledger entries as JSON (or JSONL with `--format jsonl` / `--jsonl`) |
| `clawq rooms ledger retention-cleanup --retention-days N` | Delete ledger entries older than the configured retention period (admin-only) |
| `clawq rooms gc [--retention-days N]` | Purge expired room workspaces while preserving active references (admin-only) |
| `clawq rooms bind ROOM_ID PROFILE_ID [--preserve\|--reset]` | Bind or explicitly rebind a room to a profile (admin-only) |
| `clawq rooms rename PROFILE_ID NAME` | Update a room profile display name (admin-only) |
| `clawq rooms delete PROFILE_ID [--force]` | Soft-delete a room profile and remove bindings (admin-only) |
| `clawq rooms unbind ROOM_ID` | Remove a room binding while preserving the profile (admin-only) |
...
Send a script-friendly one-off message through an already configured
connector. Telegram accepts `HTML`, `Markdown`, or `MarkdownV2`; Teams accepts
Markdown. `--account` selects a Telegram account when needed.

```bash
clawq notify --channel telegram --target 123456789 --parse-mode HTML '<b>Deploy complete</b>'
clawq notify --channel teams --target 'conversation-id' 'Deploy complete'
clawq notify --channel telegram --list-targets
clawq notify --channel teams --list-targets
```

`--list-targets` reads locally known Telegram chat IDs or Teams conversation
IDs from Clawq's SQLite session state. It does not call Telegram or Bot
Framework discovery APIs, and lists only targets the local daemon has
previously seen. Telegram target history is not account-scoped.
...
<Callout type="tip" title="Interrupt syntax">
  ...
  Use <code>/followup MESSAGE</code> to defer a message until the current turn
  finishes. Use <code>/followup-append MESSAGE</code> to append to the most
  recent queued follow-up.

  When reactions are available, Clawq acknowledges accepted busy-session
  interrupts with a salute reaction in Telegram, Discord, and Slack.
</Callout>
...
- `/followup MESSAGE` — queues a non-interrupting follow-up for delivery when the session next becomes idle; if the session is idle, sends immediately
- `/followup-append MESSAGE` — appends to the latest queued follow-up, or queues/sends a new one when none exists

When `shell_exec` is interrupted, the running process is moved to a background shell instead of being killed. Explicit `background=true` jobs behave the same way. On completion, Clawq notifies the launching agent and wakes its session if idle. Use `/tools` to see `bg_shell_status`, `bg_shell_wait`, and `bg_shell_result` for optional progress and full-output inspection.

## Slash Commands

Slash commands are available in chat-integrated channels (Telegram, Discord, Slack, Teams, web UI). Type `/help` in any channel to see core command help and the interrupt hint. Use `/help skills` for skills and `/help agents` for agents.

Model-setting commands accept canonical `provider:model`, legacy `provider/model`, or a bare model name. Bare names are matched against the model catalog; if more than one provider matches, the command fails with candidate `provider:model` values instead of choosing one silently. In the web UI, slash-command errors are also written to the session history so the next LLM turn can see what failed.

| Command | Description |
|---------|-------------|
| `/start` | Show the ready message |
| `/help` | List core slash commands and show the interrupt hint |
| `/help skills` | Show available skills |
...
| `/followup <message>` | Send a follow-up after the current turn finishes; idle sessions send immediately |
| `/followup-append <message>` | Append to the most recent queued follow-up |
...
| `/model set <name>` | Set model for this session (validates against known models; ambiguous bare names list `provider:model` candidates) |
| `/model set-force <name>` | Set model bypassing validation (for custom/local models) |
| `/model set-default <name>` | Set default model in config (persistent, auto-normalizes format; ambiguous bare names list candidates) |
