...
ambient history capture. The capture and injection paths are gated by both this
configuration and the connector capability matrix. Teams and Discord currently
support ambient capture; Slack does not, because Slack messages are already
delivered through the normal channel flow and no separate ambient-history
...
| `max_messages` | Maximum retained messages per connector-history key (1-128) |
| `max_age_days` | Retention window used by DB cleanup when persistence is enabled |
