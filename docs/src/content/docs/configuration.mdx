...
| `OPENCODEX_API_AUTH_TOKEN` | OpenCodex admission token. Must begin with `ocx_`; used after an explicit provider `api_key` and before `~/.opencodex/api-token`. |
...
|-------|----------|-------------|
| `api_key` | API-key providers only | API key (or `$ENC:...` encrypted value). Subscription/OAuth providers such as `openai-codex` can authenticate with provider-specific credentials instead. |
...
OpenCodex is a multi-provider LAN proxy with a built-in endpoint of `http://cachy.lan:10100/v1`. Configure the provider explicitly, then select a proxy-routed model with the canonical `provider:model` format:
...
Clawq resolves a valid token in this order: configured `api_key`, `OPENCODEX_API_AUTH_TOKEN`, then `~/.opencodex/api-token`. Tokens are trimmed and must start with `ocx_`; keep the token file at mode `0600`. If `cachy.lan` is unavailable, set `base_url` to `http://10.100.1.2:10100/v1`.

OpenCodex always uses the OpenAI Responses API over HTTP at `<base_url>/responses`, sends the admission token as `x-opencodex-api-key`, and never uses a websocket transport. The Codex CLI settings `wire_api`, `supports_websockets`, `env_http_headers`, and `requires_openai_auth` are therefore not Clawq config fields. Run `clawq models refresh --provider opencodex` to discover the proxy's current routes, including names such as `gpt-*` and `xai/grok-*`. Inference requires the full `clawq` binary; `clawq-min` rejects this integration-only provider.
...
`agent_defaults.primary_model` using the `provider:model` format (e.g. `"openrouter:openai/gpt-4o"`).
...
"openrouter"` + `primary_model: "openai/gpt-4o"` becomes `primary_model:
"openrouter:openai/gpt-4o"`); otherwise the redundant key is simply dropped. Either
...
Optional capture of unaddressed group-chat messages for connectors that support
ambient history capture. Persistent entries are stored with room scope,
connector type, and timestamp so cleanup and future history queries can filter
by room, connector, and time range. The capture and injection paths are gated by
both this configuration and the connector capability matrix. Teams and Discord
currently support ambient capture; Slack does not, because Slack messages are
already delivered through the normal channel flow and no separate
ambient-history capture path is implemented.
...
| Field | Description |
|-------|-------------|
| `enabled` | Enable/disable capture of unaddressed group messages for capable connectors |
| `persist_to_db` | Persist captured history in SQLite instead of relying only on in-memory buffers |
| `max_messages` | Maximum retained messages per room/connector history scope (1-128) |
| `max_age_days` | Retention window used by DB cleanup when persistence is enabled; entries older than this are deleted |

Use `/inject_connector_history [N]` or the `inject_connector_history` tool from a
Teams or Discord group session to load recent captured messages into context.
...
| `sandbox_backend` | `"auto"` | Sandbox backend for shell/runner isolation: `"auto"` (detects firejail > bubblewrap > none), `"firejail"`, `"bubblewrap"`, or `"none"` |
| `hosted_runner_isolation` | `"off"` | Isolation for hosted codex/claude background runners: `"off"`, `"prefer"` (sandbox when a backend is available), `"require"` (fail closed without one). Applies a minimal allowlisted environment plus an argv-level sandbox. See the hosted-runner-isolation operator doc. |
...
When an interactive agent calls `memory_forget`, Clawq removes the key from active memory and appends a recovery copy to `$CLAWQ_HOME/workspace/rooms/<session-slug-hash>/memory_archive.jsonl` (or `~/.clawq/workspace/rooms/<session-slug-hash>/memory_archive.jsonl`). Each JSONL row records the key, category, content, and UTC time the memory was forgotten.
...
```json
{
  "egress": {
    "strictness": "strict",
    "default_allowlist": [
      {"host": "clawq.org", "path": "/llms.txt", "method": "GET", "action": "allow", "log_policy": "no_log"},
      {"host": "clawq.org", "path": "/llms-full.txt", "method": "GET", "action": "allow", "log_policy": "no_log"}
    ]
  }
}
```
...
| Field | Default | Description |
|-------|---------|-------------|
| `strictness` | `"strict"` | `"strict"` denies unmatched policy-aware HTTP destinations; `"permissive"` allows unmatched destinations after explicit rules and the default allowlist are checked |
| `default_allowlist` | Clawq `llms.txt` and `llms-full.txt` on `clawq.org` | Fallback egress rules evaluated after access-bundle `egress_rules`; explicit deny rules in higher-priority scopes still win |
| `default_policy` | alias | Backward-friendly alias for `strictness`; `"deny"` maps to `"strict"`, `"allow"` maps to `"permissive"` |

Egress policy applies to Clawq's policy-aware HTTP paths such as `web_fetch`,
`web_search`, `http_get`, `http_request`, HTTP MCP startup/tool calls, and
other callers that use the policy HTTP client. Shell command network access is
controlled separately by the shell sandbox/workspace policy; egress rules do
not claim to firewall arbitrary commands run through `bash`.
...
Per-session workspace subdirectories may also contain runtime artifacts. Forgotten memories are archived to `memory_archive.jsonl` in the invoking session's workspace subdirectory so mistakenly removed memories can be recovered later.
