...
<Callout type="note" title="Per-session opt-in required">
  ...
  <code>clawq session heartbeat SESSION on</code>. Only Telegram, Slack, and
  Discord sessions are valid heartbeat targets.
</Callout>
...
An array of routing bindings that dispatch messages to specific agents by pattern matching. Each binding specifies a pattern to match against incoming messages and routes matching messages to a named agent configuration.
...
| Field | Default | Description |
|-------|---------|-------------|
| `summarizer_enabled` | `true` | Enable autosummarizer for long tool output |
| `summarizer_model` | `"groq:openai/gpt-oss-120b"` | Model for summarization |
